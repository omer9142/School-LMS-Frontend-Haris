...
const StatsCard = React.memo(({ icon: Icon, value, label, color, isMobile }) => (
  <Card sx={{ p: 1.5, display: 'flex', alignItems: 'center', height: '100%' }}>
    <Avatar sx={{ 
      bgcolor: `${color}15`, 
      mr: 1.5,
      width: { xs: 32, sm: 40 },
    ...
    <Box>
      <Typography variant={isMobile ? "h6" : "h5"} fontWeight="bold" color={color}>
        {value}
      </Typography>
...
const MobileDayView = React.memo(({ day, timetableGrid, currentPeriod, currentDay }) => (
  <Box>
    <Card sx={{ mb: 2, bgcolor: 'primary.main', color: 'white' }}>
      <CardContent sx={{ mt: 2, textAlign: 'center', py: 2 }}>
...
const TeacherTimetable = () => {
  ...
  // Optimize timetable grid creation with useMemo
  ...
  const stats = useMemo(() => {
    ...
    return { totalSlots, teachingSlots, freeSlots, teachingDays };
  }, [timetable]);
